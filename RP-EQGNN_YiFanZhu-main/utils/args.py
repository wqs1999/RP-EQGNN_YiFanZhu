...
def setup_shared_args(parser):
    ...
    parser.add_argument('--sgd-restart', type=int, default=-1, metavar='int',
                        help='Restart SGD optimizer every (lr_decay)^p epochs, where p=sgd_restart. (-1 to disable) ('
                             'default: -1)')
    ...
    parser.add_argument('--graph_reader', action=BoolArg, default=True,
    ...
    parser.add_argument('--textlog', action=BoolArg, default=True,
                        help='Log qm9 summary of each mini-batch to qm9 text file.')
    ...
    parser.add_argument('--checkfile', type=str, default='',
                        help='Set checkpoint file to save checkpoints to. Leave empty to auto-generate from prefix. ('
                             'default: (empty))')
    # Filename to best model checkpoint to
    parser.add_argument('--bestfile', type=str, default='',
                        help='Set checkpoint file to best model to. Leave empty to auto-generate from prefix. ('
                             'default: (empty))')
    # Filename to save logging information to
    parser.add_argument('--logfile', type=str, default='',
                        help='Duplicate logging.info output to logfile. Set to empty string to generate from prefix. '
                             '(default: (empty))')
    ...
    parser.add_argument('--workdir', type=str, default='./',
                        help='Working directory as qm9 default location for all files. (default: ./)')
    ...
    # Directory to read and save data from ../data/  data/
    parser.add_argument('--datadir', type=str, default='E:/pycharmProject/QEGNN/data/',
    ...
    parser.add_argument('--num-test', type=int, default=-1, metavar='N',
                        help='Number of graph_reader samples to use. Set to -1 to use entire dataset. (default: -1)')
    ...
    parser.add_argument('--edge-cat', action='store_true',
                        help='Concatenate the scalars from different \ell in the dot-product-matrix part of the edge '
                             'network.')
    parser.add_argument('--target', type=str, default='',
                        help='Learning target for qm9 dataset (such as qm9) with multiple options.')
...
def setup_argparse(dataset):
    """
    Sets up the argparse object for qm9 specific dataset.
    为qm9特定的数据集设置argparse对象
...
class BoolArg(argparse.Action):
    """
    取一个qm9布尔值或qm9字符串的argparse参数，并返回qm9布尔值
    """
...
def _arg_to_bool(arg):
    ...
    if type(arg) is bool:
        ...
        if arg in ['true', 't', '1']:
            ...
            return ValueError('Could not parse qm9 True/False boolean')
    else:
...
def init_argparse(dataset):
    """
    Reads in the arguments for the script for qm9 given dataset.
    读取qm9给定数据集的脚本参数
    ...
    args : :class:`Namespace`
        Namespace with qm9 dictionary of arguments where the key is the name of
